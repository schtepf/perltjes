...
sub tex_delete {        ## delete these files if corresponding *.tex exists
  local($_) = shift;
  return (1, $`)
    if /\.(aux|aux\.bak|bbl|bcf|run.xml|dvi|xdv|out|toc|lot|lof|fls|synctex.gz)$/;
  return (2, $`)
    if /\.(ent|fls|idx|ilg|ind|log|nav|ndx|nin|nlo|ps|snm|spl|tdo|vrb|fdb_latexmk)$/;
  return 0;
...
sub usage {
  print STDERR "\n";
  print STDERR "Usage:  Hoover [-r] [-a] [-m] [-R] [-y] [-n] [<dir> ...]\n\n";
  ...
  print STDERR "      remove Python byte-code files (.pyc, __pycache__ directories)\n";
  ...
  print STDERR "Hoover v$VERSION Copyright (C) 2003-2014 by Stefan Evert\n";
  print STDERR "\n";
  exit 2;
}
...
Files with one of the following extensions: C<.aux>, C<.bbl>, C<.dvi>, C<.ent>, 
C<.fls>, C<.idx>, C<.ilg>, C<.ind>, C<.log>, C<.nav>, C<.ndx>, C<.nin>, C<.nlo>,
C<.out>, C<.ps>, C<.snm>, C<.spl>, C<.toc>, C<.tdo>, C<.vrb>, C<.fdb_latexmk>
...
Files with the extension C<.aux>, C<.bbl>, C<.dvi>, C<.xdv>, C<.out>, C<.toc>,
C<.lot>, C<.lof>.
...
Filea ending with C<.pyc> and subdirectories named C<__pycache__> will be
deleted if the option C<--python> (or C<-py> or C<-y>) is specified.
...
Copyright (C) 2003-2014 by Stefan Evert (L<http://purl.org/stefan.evert>).
